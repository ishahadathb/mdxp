...
**Markdown, browsable.** Point `mdxplore` at a folder and it opens a fast, readable HTML explorer for every `.md` file inside — rendered live, on demand. Nothing is written to your files.
...
Most Markdown tools *convert* your files into a pile of `.html` you have to manage. `mdxplore` doesn't. It's a tiny local server that reads each `.md` when you open it and renders it in the browser. Close it and there's no residue — the directory you served is exactly as you left it.
...
```bash
npx mdxplore            # serve the current directory
npx mdxplore ./docs     # serve ./docs
...
- **Zero residue** — `mdxplore` never writes into the directory it serves.

## How it works

`mdxplore` starts a local HTTP server bound to `127.0.0.1`. Requests map to routes:
...
- `/assets/*` serves mdxplore's own bundled CSS/JS and Mermaid — never from your folder.
...
The version in `package.json` is the single source of truth — it's what `mdxplore --version`
...
To use the `mdxplore` command from any folder while developing, link it once:

```bash
npm link          # then run `mdxplore ./any/folder` anywhere
```
